...
_assistant_recognizer = [None,None,None,None,None,None,None,None,None,None,None,None,None]


class _AssistantRecognizer(object):
    """Your personal Google Assistant."""

    def __init__(self, credentials,stt=0):
        model_id, device_id = aiy.assistant.device_helpers.get_ids_for_service(credentials,stt)
        print(model_id, device_id)
...
def get_assistant(link=None, stt=0):
    ...
    if not _assistant_recognizer[stt]: 
        credentials = aiy.assistant.auth_helpers.get_assistant_credentials(credentials_file=link,stt=stt)
        _assistant_recognizer[stt] = _AssistantRecognizer(credentials,stt)
    return _assistant_recognizer[stt]
