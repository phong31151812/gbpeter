...
aaa=os.path.expanduser('~/acc')
_ASSISTANT_CREDENTIALS_FILE = os.path.join(aaa, '0.json')
...
def get_assistant_credentials(credentials_file=None,stt=0):
    name=str(stt)+'_assistant_credentials.json'
    global _ASSISTANT_CREDENTIALS_FILE, _ASSISTANT_CREDENTIALS
    _ASSISTANT_CREDENTIALS = os.path.join(_VR_CACHE_DIR, name)
    if not credentials_file:
        name1=str(stt)+'.json'
        _ASSISTANT_CREDENTIALS_FILE=os.path.join(aaa, name1)
        credentials_file = _ASSISTANT_CREDENTIALS_FILE
    else:
        _ASSISTANT_CREDENTIALS_FILE = credentials_file
